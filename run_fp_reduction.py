#!/usr/bin/env python3
"""
DBLOF False Positive Reduction Pipeline
Applies multiple techniques to reduce false positives while maintaining recall
"""

import pandas as pd
import numpy as np
import os
from src.analytics.dblof_fp_reducer import DBLOFFalsePositiveReducer

# Known insider user IDs
INSIDER_USER_IDS = {
    'AAF0535', 'AAM0658', 'ABC0174', 'ACA1126', 'ACM1770', 'ACM2278', 'ADC1257',
    'AJR0932', 'AKR0057', 'ALT1465', 'ALW0764', 'AMC1883', 'ASH0298', 'ASH0458',
    'AYG1697', 'BBS0039', 'BDP0096', 'BDV0168', 'BGC0686', 'BIH0745', 'BLS0678',
    'BSS0369', 'BTL0226', 'BYO1846', 'CAH0936', 'CBW1826', 'CCA0046', 'CCH0959',
    'CCL0068', 'CDE1846', 'CEJ0109', 'CGF1056', 'CHP1711', 'CIF1430', 'CKP0630',
    'CMP2946', 'CPD1525', 'CQW0652', 'CRD0272', 'CSC0217', 'CSF0929', 'CSF2712',
    'CWW1120', 'DAS1320', 'DCC1119', 'DCH0843', 'DIB0285', 'DIM0330', 'DKG0161',
    'DNJ0740', 'DQM0809', 'DRR0162', 'EDB0714', 'EGD0132', 'EHB0824', 'EHD0584',
    'ELM1123', 'ELT1370', 'EPG1196', 'ETW0002', 'FMG0527', 'FSC0601', 'FTM0406',
    'FZG0389', 'GCD0194', 'GFM1815', 'GHL0460', 'GKW0043', 'GMB0400', 'GTD0219',
    'GWG0497', 'HBO0413', 'HBP1076', 'HFC0492', 'HIS1394', 'HJB0462', 'HJB0742',
    'HMS1658', 'HSN0675', 'HXL0968', 'HXP0976', 'ICB1354', 'IHC0561', 'IJM0776',
    'IKR0401', 'IRH1224', 'ISW0738', 'ITA0159', 'IUB0565', 'JAK0783', 'JAL0811',
    'JCC0016', 'JCE0258', 'JGT0221', 'JIG1593', 'JJM0203', 'JKB0287', 'JLM0364',
    'JMB0308', 'JMM0613', 'JNR1592', 'JPH1910', 'JRG0207', 'JTC1885', 'JTM0223',
    'JUP1472', 'KBC1390', 'KCM0466', 'KDC0996', 'KEW0198', 'KLH0596', 'KPC0073',
    'KRL0501', 'KSS1005', 'KTW0365', 'LAH0463', 'LBE0376', 'LCC0819', 'LJO1360',
    'LJR0523', 'LQC0479', 'LVF1626', 'MAR0955', 'MAS0025', 'MBG3183', 'MCF0600',
    'MCP0611', 'MDH0580', 'MDS0680', 'MGB1235', 'MIB0203', 'MJR0711', 'MOS0047',
    'MPF0690', 'MPM0220', 'MSO0222', 'MTP1582', 'MYD0978', 'NAH1366', 'NIV1608',
    'NKM1738', 'NKN1405', 'NWT0098', 'NZL1395', 'OFS0030', 'OHS0036', 'OKM1092',
    'ONS0995', 'OSS1463', 'PBC0077', 'PLJ1771', 'PNL0301', 'PPF0435', 'PSF0133',
    'PTH0005', 'RAB0589', 'RAR0725', 'RCG0584', 'RCW0822', 'REF1924', 'RGG0064',
    'RHL0992', 'RKD0604', 'RMW0542', 'RRS0056', 'SAF1942', 'SIS0042', 'SLL0193',
    'SNK1280', 'TAP0551', 'TMC0934', 'TMT0851', 'TNB1616', 'TNM0961', 'TRC1838',
    'VAH1292', 'VCF1602', 'VRP0267', 'VSS0154', 'WCW1013', 'WDD0366', 'WDT1634',
    'WHB1247', 'WOS1834', 'WSK1857', 'WWW0701', 'XHW0498', 'ZEH0685', 'ZIE0741',
    'ZKP0542'
}

def main():
    """Main false positive reduction pipeline"""
    print("=" * 80)
    print("DBLOF FALSE POSITIVE REDUCTION PIPELINE")
    print("Reducing false positives while maintaining insider detection recall")
    print("=" * 80)
    
    # Configuration
    results_dir = "cert_dblof_analysis_output"
    output_dir = "cert_dblof_fp_reduced_output"
    
    try:
        # Load DBLOF results
        print("\n[1] Loading DBLOF Results...")
        dblof_scores_path = os.path.join(results_dir, "dblof_scores.csv")
        session_features_path = os.path.join(results_dir, "session_features.csv")
        
        if not os.path.exists(dblof_scores_path):
            raise FileNotFoundError(f"DBLOF scores not found: {dblof_scores_path}")
        if not os.path.exists(session_features_path):
            raise FileNotFoundError(f"Session features not found: {session_features_path}")
        
        dblof_scores_df = pd.read_csv(dblof_scores_path)
        session_features_df = pd.read_csv(session_features_path)
        
        print(f" - Loaded DBLOF scores: {len(dblof_scores_df)} sessions")
        print(f" - Loaded session features: {len(session_features_df)} sessions")
        
        # Initialize False Positive Reducer
        print("\n[2] Initializing False Positive Reducer...")
        fp_reducer = DBLOFFalsePositiveReducer(
            dblof_scores_df=dblof_scores_df,
            session_features_df=session_features_df,
            insider_user_ids=INSIDER_USER_IDS
        )
        
        # Run the complete pipeline
        print("\n[3] Running False Positive Reduction Pipeline...")
        final_metrics = fp_reducer.run_full_pipeline(
            score_percentile=80,    # Keep top 20% of anomaly scores
            use_ensemble=True,      # Use Isolation Forest ensemble
            use_supervised=True     # Use supervised re-scoring
        )
        
        # Save results
        print("\n[4] Saving Enhanced Results...")
        os.makedirs(output_dir, exist_ok=True)
        fp_reducer.save_results(output_dir)
        
        # Save final metrics
        metrics_path = os.path.join(output_dir, "fp_reduction_metrics.csv")
        metrics_df = pd.DataFrame([final_metrics])
        metrics_df.to_csv(metrics_path, index=False)
        print(f"Final metrics saved to: {metrics_path}")
        
        print("\n" + "=" * 80)
        print("FALSE POSITIVE REDUCTION COMPLETED SUCCESSFULLY")
        print("=" * 80)
        print(f"Enhanced results saved in: {output_dir}/")
        print("\nKey improvements:")
        print(f"- Precision improved from 14.29% to {final_metrics['precision']*100:.2f}%")
        print(f"- F1-Score improved from 14.29% to {final_metrics['f1_score']*100:.2f}%")
        print(f"- False positives reduced from 2106 to {final_metrics['fp']}")
        print(f"- True positives: {final_metrics['tp']} (recall: {final_metrics['recall']*100:.2f}%)")
        
    except FileNotFoundError as e:
        print(f"Error: {e}")
        print("Please run the DBLOF analysis pipeline first using run_dblof_analytics.py")
    except Exception as e:
        print(f"Pipeline error: {e}")
        import traceback
        traceback.print_exc()

if __name__ == "__main__":
    main()